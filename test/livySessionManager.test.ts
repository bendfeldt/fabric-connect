...
function manager(api: IFabricApiClient, store: SessionStore): LivySessionManager {
...
test('reattaches to a persisted live session instead of starting fresh', async () => {
  ...
  const store = memoryStore({
    [`livy-session/${TARGET.tenantId}/${TARGET.workspaceId}/${TARGET.lakehouseId}`]: '42',
  });
  const result = await manager(api, store).execute(
    TARGET,
    'x',
  ...
  assert.equal(result.status, 'ok');
  assert.ok(!api.calls.some((c) => /POST .*\/sessions$/.test(`${c.method} ${c.path}`)));
});
...
test('session start failure is an actionable infra error, distinct from cell errors', async () => {
  ...
  await assert.rejects(
    manager(api, memoryStore()).execute(TARGET, 'x', 'pyspark', NEVER_CANCELLED),
    (error: unknown) => {
...
test('executions on the same session are queued, not raced', async () => {
  ...
  const api = scriptedApi([
    ...
    [
      ...
      (options) => {
        ...
        return { id: Number(id), state: 'available', output: { status: 'ok', data: {} } };
      },
    ],
  ]);
  ...
  assert.deepEqual(events, ['start:first', 'finish:0', 'start:second', 'finish:1']);
});
...
test('stopSession deletes the session and clears persisted state', async () => {
  const store = memoryStore({
    [`livy-session/${TARGET.tenantId}/${TARGET.workspaceId}/${TARGET.lakehouseId}`]: '9',
  });
